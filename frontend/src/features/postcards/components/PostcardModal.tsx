...
export function PostcardModal({ postcard, onClose, eventLogoUrl, theme }: PostcardModalProps) {
  ...
  const presentation = postcard ? getPostcardPresentation(postcard) : null;

  // Orientation hook for photo mode
  const mediaSrc = isVideo && postcard
    ? (postcard.thumbnail_path || postcard.image_path)
    : postcard?.image_path;
  ...
  const modalLabel = postcard
    ? `Postal de ${presentation?.captionName || postcard.player_name}`
    : 'Postal';
      ...
      {postcard && presentation && (
        ...
        >
          ...
          <motion.div
            ...
            tabIndex={-1}
          >
            {/* Photo mode: no pin, FramedPhoto centered */}
            {presentation.mode === 'photo' ? (
              <div
                ref={postcardRef}
                className="shadow-2xl overflow-hidden p-6"
                style={{
